  ...
  run() {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      const fakeUser = {
        ...
        firstname: this.faker.person.firstName(), // Generate a fake first name
        lastname: this.faker.person.lastName(), // Generate a fake last name
        refName: `user_${i}`, // Create a reference name for the user
      };

      // Insert the fakeUser data into the 'user' table
      this.insert(fakeUser); // insert into user(email, password, firstname, lastname) values (?, ?, ?, ?)
    }
  }
}
